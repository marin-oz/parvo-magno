...
class_file_path = "../data/synsets_table.csv"
class_file = pd.read_csv(class_file_path, index_col=0)
...
categories16 = {
    ...
    "chair":    ['n02791124', 'n03376595', 'n04099969', 'n00605023', 
                 'n04429376'],
    "bear":     ['n02132136', 'n02133161', 'n02134084', 'n02134418'],
    "boat":     ['n02951358', 'n03344393', 'n03662601', 'n04273569', 
                 'n04612373', 'n04612504'],
    "cat":      ["n02122878", "n02123045", "n02123159", "n02126465",
                 "n02123394", "n02123597", "n02124075", "n02125311"],
    "bottle":   ['n02823428', 'n03937543', 'n03983396', 'n04557648', 
                 'n04560804', 'n04579145', 'n04591713'],
    "truck":    ['n03345487', 'n03417042', 'n03770679', 'n03796401', 
                 'n00319176', 'n01016201', 'n03930630', 'n03930777', 
                 'n05061003', 'n06547832', 'n10432053', 'n03977966',
                 'n04461696', 'n04467665'],
    "car":      ['n02814533', 'n03100240', 'n13419325', 'n04285008'], #'n03100346' sofabed removed
...
category_names = categories16.keys()
categories16_class_indices = {}
for category in categories16.keys():
    list_wnid = categories16[category]
    list_index =[]
    for wnid in list_wnid:
        match = class_file.index[class_file['WNID']==wnid].values
        if len(match) > 0:
            assert len(match)==1, "more than 1 match found"
            list_index.append(match[0]-1) # synset table index starts from 1
        else:
            print("no match for " + wnid)
        
    categories16_class_indices.update({category: list_index})

# save the categories16_class_indices to a pickle file
with open("../data/categories16_class_indices.pkl", "wb") as f:
    pickle.dump(categories16_class_indices, f)
